 ...
 *
 * Depth pass (2026-07-28): the original hooks ('how it actually works',
 * 'one decision', 'the trick nobody taught you', 'sounds impossible but')
 * were generic curiosity phrases, not topics -- the LLM had to invent a
 * subject to hang them on every time, and it kept reaching for the same
 * safe, shallow, already-covered-everywhere ones ("how large language
 * models actually work" beat almost every other candidate, run after run,
 * because it's the most search-familiar phrase, not because it's the best
 * topic). Rewritten so each hook NAMES a specific, substantive, under-
 * covered angle rather than a vague curiosity template -- content that's
 * genuinely deep and least-touched, not 101-level explainers.
 */
export const PROVEN_GROWTH_NICHES = [
  {
    ...
    hooks: [
      'why large language models hallucinate (the actual mechanism, not "they guess")',
      'mechanistic interpretability: what happens when researchers open up a neural network',
      'the bitter lesson: why raw compute keeps beating clever human-designed methods',
      'catastrophic forgetting: why AI models can\'t learn new things without erasing old ones'
    ]
  },
  {
    ...
    hooks: [
      ...
      'the overlooked rivalry between two lesser-known figures that reshaped a field'
    ]
  },
  {
    id: 'daily-life-hack',
  ...
  },
  {
    ...
    hooks: [
      'the measurement problem in quantum mechanics that still has no agreed answer',
      'the black hole information paradox and why it broke physics for 50 years',
      'why entropy only increases: the actual reason time has a direction',
      'Bell\'s theorem: the experiment that ruled out a whole category of explanations for reality'
    ]
  }
];
...
export function buildGrowthDescription({
  ...
  // Topic-specific hashtags (from the LLM + topicHashtags) come first so
  // they survive the cap; generic branded ones fill any remaining slots.
  // Raised 8 -> 10: still comfortably under YouTube's ~15-hashtag spam
  // threshold, with room for more topic-derived tags than before.
  const tags = [...new Set([
...
- Deliver ONE clear payoff before the 20-second mark, THEN keep going -- a real Short here runs 50-90 seconds, so the payoff is the hook that earns the rest of the watch time, not the end of the video.
- After the initial payoff, add a concrete example, the "why" behind it, or a twist most explanations miss -- genuine additional substance, not restating the hook in different words.
- End with a soft loop CTA: ask a yes/no question or "follow for part 2" style line — never hard-sell.
- Spoken length target: 50-90 seconds (about 160-240 words). Longer here means more real substance, never filler or padding -- if you run out of genuinely new material, stop short rather than pad.`;
  }
  return `
...
/** `forceId` lets a manual/backfill run (daily.yml's `niche` workflow_dispatch
 * input, plumbed through orchestrator.js's --niche= flag) pick a specific
 * pillar for one run instead of the deterministic day-of-year rotation --
 * added after a same-day retry needed a different pillar than the one
 * already used earlier that day, which the date-keyed rotation alone can't
 * give you (it always returns the same pillar for the same calendar day). */
...
/** Programmatic, topic-derived hashtags as a guaranteed supplement to
 * whatever the LLM returns -- the LLM's hashtag count is a request, not a
 * guarantee, and generic ones ("Explained", "Facts") don't help discovery
 * as much as the topic's own specific nouns do. Mirrors buildGrowthTags'
 * topicBits extraction but formatted as hashtag-safe single words. */
