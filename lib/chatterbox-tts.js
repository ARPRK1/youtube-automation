...
/**
 * Terminate the persistent Chatterbox Python server. CRITICAL: without this
 * the spawned python child keeps the Node event loop alive, so
 * `node orchestrator.js` never exits after generation finishes and the
 * GitHub Actions job runs until its 240-min timeout and is marked FAILED
 * (confirmed live 2026-08-13: real work took 20 min, the process then hung
 * 3h40m; the runner logged "Terminate orphan process: pid (node)/(python)").
 * Safe to call when no server was ever started. Idempotent.
 */
export function shutdownChatterbox() {
  if (!serverHandle) return;
  const { child } = serverHandle;
  serverHandle = null;
  try { child.stdin.end(); } catch { /* already closed */ }
  try { child.kill('SIGTERM'); } catch { /* already gone */ }
  // Hard-kill shortly after in case SIGTERM is ignored, and unref so this
  // timer itself can't keep the loop alive.
  try {
