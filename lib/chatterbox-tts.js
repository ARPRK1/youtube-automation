 ...
 */
export async function synthesizeWithChatterbox({ text, outputPath, referencePath, exaggeration = 0.5, cfgWeight = 0.5 }) {
  const server = startServer();
  await server.ready;
  return new Promise((resolve, reject) => {
    server.pending.push({ resolve, reject });
    server.child.stdin.write(JSON.stringify({
      text, output: outputPath, reference: referencePath, exaggeration, cfg_weight: cfgWeight
    }) + '\n');
  });
}
