...
export async function renderThumbnail({
  ...
  // Shorts: fewer chars/line, punchier mobile type (looks less "template dump")
  const lines = wrapTitle(title, kind === 'short' ? 12 : 16, kind === 'short' ? 2 : 3);
  const fontSize = kind === 'short'
    ? (lines.length <= 1 ? 100 : 78)
    : fontSizeFor(lines);
  ...
  // Detect Top N for a big number callout (human YouTube thumbnail habit)
  const topMatch = String(title).match(/\btop\s*(\d+)\b/i);
  const bigNumber = topMatch ? topMatch[1] : null;
  ...
  // Human-made look: stronger contrast, film-ish warmth, thicker type, number badge.
  // Avoid thin pastel AI-template aesthetics.
  const filters = [
    `scale=${W}:${H}:force_original_aspect_ratio=increase`,
    `crop=${W}:${H}`,
    `eq=contrast=1.28:saturation=1.22:brightness=0.02`,
    `unsharp=5:5:0.8:3:3:0.4`,
    `vignette=PI/4`,
    // layered bottom shade (reads as a gradient without geq complexity)
    `drawbox=x=0:y=${panelY - 40}:w=${W}:h=${panelH + 40}:color=black@0.4:t=fill`,
    `drawbox=x=0:y=${panelY}:w=${W}:h=${panelH}:color=black@0.72:t=fill`,
    `drawbox=x=0:y=${panelY}:w=${W}:h=8:color=${hexToFfmpegColor(accent, 1)}:t=fill`,
    `drawbox=x=0:y=0:w=18:h=${H}:color=${hexToFfmpegColor(accent, 1)}:t=fill`,
    `drawbox=x=0:y=${H - 12}:w=${W}:h=12:color=${hexToFfmpegColor(accent, 1)}:t=fill`,
    `drawtext=${fontOpt}:text='${textBlock}':fontcolor=white:fontsize=${fontSize}:` +
      `x=(w-text_w)/2:y=${textY}:line_spacing=${lineSpacing}:` +
      `borderw=6:bordercolor=black@0.95:shadowx=4:shadowy=4:shadowcolor=black@0.6`
  ];

  // Big number stamp for Top 5 / Top 10 (reads "designed", not auto-slop)
  if (bigNumber) {
    const num = escapeDrawtext(bigNumber);
    filters.push(
      `drawbox=x=28:y=28:w=140:h=140:color=${hexToFfmpegColor(accent, 0.95)}:t=fill`,
      `drawtext=${fontOpt}:text='${num}':fontcolor=white:fontsize=96:` +
        `x=28+(140-text_w)/2:y=48:borderw=3:bordercolor=black@0.5`
    );
  }

  const autoBadge = badge || (kind === 'short' ? (bigNumber ? null : 'FACT') : null);
  if (autoBadge) {
    const label = escapeDrawtext(String(autoBadge).toUpperCase().slice(0, 12));
