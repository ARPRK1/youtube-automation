 ...
 * Locale defaults to global English (en-US); the channel is a GLOBAL curiosity
 * channel (2026-08-12 pivot), so India-localized results (the old en-IN/IN
 * default) skewed fact corroboration toward Indian coverage of universal
 * topics. Override via config.topics.region if a regional slant is wanted. */
export async function fetchGoogleNewsHeadlines(query, limit = 5, region = 'US') {
  const gl = String(region || 'US').toUpperCase();
  const url = `https://news.google.com/rss/search?q=${encodeURIComponent(query)}&hl=en-${gl}&gl=${gl}&ceid=${gl}:en`;
