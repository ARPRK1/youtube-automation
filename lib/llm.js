...
async function callGroq(prompt, maxTokens, keyIndex = 0, attempt = 1, keysTried = 0) {
  ...
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      // gpt-oss is a REASONING model: without capping the reasoning it spends
      // the whole token budget "thinking" and returns EMPTY content (confirmed
      // live 2026-08-18). 'low' leaves plenty of room for the actual answer and
      // keeps scripts/JSON reliable. Ignored by any non-reasoning model.
