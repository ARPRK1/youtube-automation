...
function parseRetryDelaySeconds(text) {
  ...
  const secOnly = text.match(/try again in\s+([\d.]+)s/i);
 ...
 * (the daily cap) fails fast instead so the caller falls through to
 * Gemini immediately rather than blocking the whole pipeline. */
async function callGroq(prompt, maxTokens, attempt = 1) {
  ...
  if (res.status === 429 && attempt < 2) {
    ...
    throw new Error(`Groq API error 429 (wait too long to retry inline, ${delaySec ?? 'unknown'}s): ${body}`);
  }
...
async function callGroqVision(prompt, imageBuffer, mimeType, maxTokens, attempt = 1) {
  ...
  if (res.status === 429 && attempt < 2) {
    ...
    throw new Error(`Groq vision API error 429 (wait too long to retry inline, ${delaySec ?? 'unknown'}s): ${body}`);
  }
