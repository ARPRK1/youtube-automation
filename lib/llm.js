...
// Groq deprecated llama-3.3-70b-versatile (and llama-3.1-8b-instant) — every
// script/research call 404'd, failing the 2026-08-17 run entirely. Live model
// list on 2026-08-18 has openai/gpt-oss-120b as the strongest general text
// model; verified it returns clean JSON + prose. If Groq deprecates this too,
// re-check https://console.groq.com/docs/models and swap here.
...
// Extra tokens added to every Groq call so a reasoning model's "thinking"
// doesn't eat into the caller's requested OUTPUT budget (see callGroq).
const REASONING_HEADROOM_TOKENS = 2500;
...
async function callGroq(prompt, maxTokens, keyIndex = 0, attempt = 1, keysTried = 0) {
  ...
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.8,
      // gpt-oss is a REASONING model: reasoning tokens are drawn from
      // max_tokens BEFORE the answer, so a caller's budget got eaten and the
      // JSON/prose was truncated ("No JSON found" on big prompts, 2026-08-18).
      // Add a fixed reasoning headroom so the caller's requested budget is what
      // remains for the actual OUTPUT. 'low' keeps reasoning short; both are
...
export function extractJson(text) {
  ...
  let withoutThink = String(text).replace(/<think>[\s\S]*?<\/think>/gi, '').replace(/<think>[\s\S]*/i, '');
  // A closed ```json … ``` fence is the clean case. If the fence OPENED but was
  // never closed (the model got truncated mid-JSON), drop the opening fence so
  // the brace scan below still finds the JSON instead of choking on backticks.
  const fenced = withoutThink.match(/```(?:json)?\s*([\s\S]*?)```/i);
  let raw = fenced ? fenced[1] : withoutThink.replace(/```(?:json)?\s*/i, '');
  const start = Math.min(...['{', '['].map((c) => (raw.indexOf(c) === -1 ? Infinity : raw.indexOf(c))));
  if (!isFinite(start)) throw new Error(`No JSON found in LLM output: ${text.slice(0, 300)}`);
  const isArray = raw[start] === '[';
  let end = isArray ? raw.lastIndexOf(']') : raw.lastIndexOf('}');
  // Truncated output (no closing bracket): salvage by closing open braces so a
  // partial-but-usable object survives instead of failing the whole call.
  let slice;
  if (end === -1) {
    slice = closeTruncatedJson(raw.slice(start));
  } else {
    slice = raw.slice(start, end + 1);
  }
  const attempts = [slice, slice.replace(/,(\s*[\]}])/g, '$1'), closeTruncatedJson(slice)];
  let lastErr;
  for (const s of attempts) {
    try { return JSON.parse(s); } catch (e) { lastErr = e; }
  }
  throw new Error(`Unparseable JSON in LLM output (${lastErr?.message}): ${text.slice(0, 200)}`);
}

/** Best-effort close of a truncated JSON string: drops any trailing partial
 * token, then appends the closing ] / } implied by the still-open brackets.
 * Turns "…{\"a\":1},{\"b\":" into "…{\"a\":1}]" so a truncated concepts/shorts
 * array still yields the complete entries instead of throwing. */
function closeTruncatedJson(s) {
  let str = String(s);
  // Cut back to the last structurally-safe boundary (a closed } or ] or a quote).
  const lastBrace = Math.max(str.lastIndexOf('}'), str.lastIndexOf(']'));
  if (lastBrace > 0) str = str.slice(0, lastBrace + 1);
  str = str.replace(/,\s*$/, '');
  const stack = [];
