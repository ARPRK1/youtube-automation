...
async function scoreCandidates(candidates) {
  ...
  const growthNiche = pickGrowthNicheForDay();
 ...
 */
export async function researchTodaysTopic(date = new Date(), { excludeTitles } = {}) {
  ...
  const growthNiche = pickGrowthNicheForDay(date);
  const fallbackPool = allFallbackTopics();
