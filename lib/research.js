...
async function scoreCandidates(candidates) {
  ...
  const prompt = `You are a YouTube growth strategist for ModernMonk — an India-focused Shorts-first channel aiming for monetization. Preferred content pillars (boost these): Indian food origin stories, simple Indian money habits, India history with a twist, hidden India places. Today's preferred pillar: ${growthNiche.name} (${growthNiche.why}).
...
- Evergreen search + share potential for Indian mobile audience (not pure 24h score spam)
- Pattern-interrupt hook potential in the first second of a Short
- Fit to food / money / history-twist / hidden-place pillars (score these higher)
- SUBSTANCE: enough real depth for a non-repetitive long script AND at least 3 distinct Short angles. Thin one-fact topics (daily lottery, one match score) score low.
