...
async function fetchYoutubeTrending(limit = 15, region = 'US') {
  if (!hasYoutubeCredentials()) return [];
  try {
    const auth = getOAuthClient();
    const youtube = google.youtube({ version: 'v3', auth });
    // Region-neutral for the global curiosity channel (2026-08-12 pivot).
    // config.topics.region controls this; default US = broadest English feed.
    const res = await youtube.videos.list({ part: ['snippet'], chart: 'mostPopular', regionCode: String(region || 'US').toUpperCase(), maxResults: limit });
...
async function scoreCandidates(candidates, forceNicheId = null) {
  ...
  const prompt = `You are a YouTube growth strategist for ModernMonk — a GLOBAL, faceless Shorts channel of sharp curiosities. Preferred pillars: world facts, Top 5/10 lists, history with a twist, science curiosities, riddles/quizzes (food origin stories occasional). Today's preferred pillar: ${growthNiche.name} (${growthNiche.why}).
The channel runs fully automatically with NO human review before publishing.

POSITIONING: universal "wait, what?" Shorts for a global mobile audience. Prefer topics that are:
- VISUAL (objects, places, animals, maps, inventions — not abstract lectures)
- SHAREABLE (Top 5, myth-bust, riddle, surprising fact)
- INFINITE (lists and facts never run out)
Score near 0: celebrity gossip, sports live scores, Marvel/Spider-Man spam, gambling, crime victims by name.

For each candidate, decide "sensitive": true for crime victims by name, sexual abuse, self-harm, mass tragedy, terrorism, minors in abuse, or anything tasteless for unsupervised publish. Also sensitive: gambling/lottery promotion.

For non-sensitive, score 0-10 for Shorts growth:
- Visual potential (can we show real objects/places without random faces?)
- Pattern-interrupt hook in second 1
- Evergreen global share (not pure 24h news)
- Fit to facts / top-lists / history-twist / science-curiosity / quiz
- Enough for 3 distinct Short angles
 ...
 */
export async function researchTodaysTopic(date = new Date(), { excludeTitles, forceNicheId, forceTopic } = {}) {
  const config = loadConfig();
  const dateStr = date.toISOString().slice(0, 10);
  const runDir = path.join(config.paths?.runs_dir || 'runs', dateStr);
  await mkdir(runDir, { recursive: true });
  const region = config.topics?.region || 'US';

  // Pinned topic (orchestrator --topic="..."): skip candidate scoring entirely
  // and build the research object directly around this exact topic, still
  // pulling real Google News headlines for factual grounding. Used to produce
  // a specific flagship video on a chosen subject instead of the daily pick.
  if (forceTopic) {
    if (isHardExcluded(forceTopic) || NSFW_VISUAL_RE.test(forceTopic)) {
      throw new Error(`Pinned --topic "${forceTopic}" is blocked (sensitive/off-brand/anatomical)`);
    }
    const headlines = await fetchGoogleNewsHeadlines(forceTopic, 6, region).catch(() => []);
    const research = {
      date: dateStr,
      topic: forceTopic,
      score: 10,
      reason: 'pinned topic (--topic override)',
      source: 'pinned',
      facts: headlines.map((h) => ({ text: h.title, source: h.source, url: h.link })),
      allCandidates: [{ title: forceTopic, score: 10 }],
      excludedForSafety: []
    };
    await writeFile(path.join(runDir, 'research.md'),
      `# Research (pinned): ${forceTopic}\n\n**Date:** ${dateStr}\n**Source:** pinned via --topic\n\n## Key facts\n\n${research.facts.length ? research.facts.map((f) => `- ${f.text}${f.source ? ` (${f.source})` : ''}`).join('\n') : '- (no corroborating headlines — script uses topic + general knowledge)'}\n`,
      'utf-8');
    const history = await loadHistory(config);
    history.push({ topic: forceTopic, date: dateStr });
  ...
  const [trends, ytTrending] = await Promise.all([
    ...
    wantYoutube ? fetchYoutubeTrending(15, region) : Promise.resolve([])
  ]);
  ...
  for (const candidate of ranked.slice(0, 8)) {
    const found = await fetchGoogleNewsHeadlines(candidate.title, 5, region).catch(() => []);
