...
import { nicheBoostForTitle, pickGrowthNicheForDay } from './growth.js';
...
async function fetchYoutubeTrending(limit = 15) {
  ...
  try {
    ...
    // Was regionCode: 'IN' -- same India-leftover issue as
    // google_trends_india above; US is the more broadly-relevant/universal
    // signal for this channel's current global-audience positioning.
    const res = await youtube.videos.list({ part: ['snippet'], chart: 'mostPopular', regionCode: 'US', maxResults: limit });
...
async function scoreCandidates(candidates, forceNicheId = null) {
  ...
  const prompt = `You are a YouTube growth strategist for ModernMonk — a global, faceless Shorts-first explainer channel aiming for monetization. Preferred content pillars (boost these): AI & ML explained simply, history turning points (one decision that changed everything), daily-life hacks, and physics/logic paradoxes. Today's preferred pillar: ${growthNiche.name} (${growthNiche.why}).
...
POSITIONING (important, applies to every pillar): this channel goes DEEP, not 101-level. The goal is advanced, substantive, under-covered topics that are genuinely important but rarely explained well -- not beginner explainers that have already been made thousands of times ("how does AI work", "what is a black hole", "5 productivity tips"). Prefer a specific mechanism, named phenomenon, real research finding, or overlooked-but-consequential event over a broad, generic survey of a topic. If a candidate title is a well-worn, oversaturated 101 phrase, score it lower even if it technically fits a pillar -- the bar is "would a genuinely curious, already-somewhat-informed viewer learn something they haven't seen explained a hundred times already."
...
For everything NOT sensitive, score 0-10 on overall potential for a genuinely good Shorts + long video, weighing:
- DEPTH: is this a specific, advanced, under-covered angle (score high) or a generic, oft-repeated 101-level survey (score low), per the positioning above
- How well it fits a whiteboard/diagram/sketch visual style: can the idea be drawn, diagrammed, or illustrated as a concept, versus needing real location-specific photos/footage that don't exist for it
- Evergreen search + share potential for a global audience (not pure 24h news spam, not region-locked)
- Pattern-interrupt hook potential in the first second of a Short
- Fit to AI/ML / history-turning-point / daily-hack / physics-paradox pillars (score these higher)
- SUBSTANCE: enough real depth for a non-repetitive long script AND at least 3 distinct Short angles. Thin one-fact topics (single news headline, one score/result) score low.
...
export async function researchTodaysTopic(date = new Date(), { excludeTitles, forceNicheId } = {}) {
  ...
  // google_trends_india was always fetched regardless of this flag until
  // 2026-07-29 -- a leftover from before the channel's India-focused ->
  // universal/global pivot (2026-07-24). Confirmed live as the direct cause
  // of a same-day failure: with on-pillar backup topics exhausted, this
  // fed "Batwara 1947" (India's Partition, off-brand for the new universal
  // positioning and a poor fit for the depth-explainer script format) in
  // as backup topic 2/2, which then failed generation. Off by default now.
  ...
  // Growth-focused evergreen bank: prioritize the day's preferred pillar
  // (food / money / history / places) then mix other curated topics so a
  // thin trending day still has substantial alternatives.
  const growthNiche = pickGrowthNicheForDay(date, forceNicheId);
  const fallbackPool = allFallbackTopics(forceNicheId);
  const dayOfYear = Math.floor((date - Date.UTC(date.getUTCFullYear(), 0, 0)) / 86400000);
  // Seed 4 hook-shaped titles from the preferred pillar
  for (const hook of (growthNiche.hooks || []).slice(0, 4)) {
    const title = `${hook} — ${growthNiche.name}`;
    if (seen.has(title.toLowerCase())) continue;
    seen.add(title.toLowerCase());
    candidates.push({ title, rawSource: 'growth-pillar', growthNiche: growthNiche.id });
  }
  for (let i = 0; i < 10 && fallbackPool.length > 0; i++) {
    const title = fallbackPool[(dayOfYear + i) % fallbackPool.length];
    if (seen.has(title.toLowerCase())) continue;
    seen.add(title.toLowerCase());
