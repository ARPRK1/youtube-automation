...
export async function researchTodaysTopic(date = new Date(), { excludeTitles } = {}) {
  ...
  // Mix in a daily-rotating slice of curated evergreen topics so a thin
  // trending day (single-fact items like a lottery result or one day's
  // gold price -- both seen scoring low but still winning by default in
  // production testing) has real substantial alternatives to lose to,
  // instead of winning purely because nothing else was on offer.
  ...
  for (const candidate of ranked.slice(0, 8)) {
    ...
    if (found.length > bestCheckedHeadlines.length) { bestChecked = candidate; bestCheckedHeadlines = found; }
    if (found.length >= 4) {
      winner = candidate;
      headlines = found;
      break;
  ...
  if (!winner) {
    ...
    // or re-fetching one already tried in the loop above.
    winner = bestChecked;
    headlines = bestCheckedHeadlines;
  }
