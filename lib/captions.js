...
/**
 * @param {object} opts
 * @param {'vertical'|'landscape'} [opts.aspect]  Shorts get fewer words/line
 *   so captions stay a single compact strip instead of wrapping into a wall.
 */
export async function buildSegmentCaptions({ srtPath, text, durationSec, outPath, aspect = 'landscape' }) {
  // Vertical Shorts: 3–4 words max keeps one thin line. Landscape can hold more.
  const maxWords = aspect === 'vertical' ? 4 : 6;
  let lines;
  if (srtPath) {
    const cues = await parseSrt(srtPath);
    lines = cues.length > 0
      ? splitIntoShortLines(cues, maxWords)
      : evenlyDistributedLines(text, durationSec, maxWords);
  } else {
    lines = evenlyDistributedLines(text, durationSec, maxWords);
  }
