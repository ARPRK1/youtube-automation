...
// The ffmpeg `subtitles` filter needs to be told what resolution the SRT's
// implied coordinate space is (`original_size`) -- without it, libass falls
// back to its own default reference resolution instead of the actual output
// frame, and the resulting scale-up lands completely differently on a wide
// 1920x1080 long-form frame than a tall 1080x1920 Shorts frame. That
// mismatch (not a deliberate style choice) is why Shorts captions were
// ballooning to take up half the screen while long-form looked fine.
// FontSize/MarginV below are tuned per-aspect against the real output
// resolution now that original_size makes the scaling predictable.
function captionStyle(aspect) {
  return aspect === 'vertical'
    ? "FontName=Arial,FontSize=64,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=3,Outline=3,Shadow=0,Alignment=2,MarginV=140"
    : "FontName=Arial,FontSize=52,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=3,Outline=2,Shadow=0,Alignment=2,MarginV=70";
}
