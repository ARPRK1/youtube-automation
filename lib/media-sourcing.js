...
export async function expandSegmentVisualBeats({
  ...
  // Animation-first: prefer real MOTION b-roll over still images (owner ask).
  const preferVideo = config.media?.prefer_video !== false;
  const scratch = (verifyAiBeats || preferVideo) ? await mkdtemp(path.join(tmpdir(), 'ytauto-beatvis-')) : null;

  // Build a small pool of real motion clips, one per distinct entity (capped so
  // the extra sourcing stays bounded). Beats then prefer a matching clip and
  // only fall back to a still (real reuse or AI) when no clip is available —
  // exactly the "animation, else images" ladder the owner asked for.
  const usedBrollUrls = new Set();
  const videoByEntity = new Map();
  if (preferVideo) {
    if (primaryAsset?.type === 'video' && primaryAsset.buffer) {
      videoByEntity.set(primaryAsset.entity, primaryAsset);
      if (primaryAsset.url) usedBrollUrls.add(primaryAsset.url);
    }
    const uniqueEntities = [...new Set(entities)].slice(0, config.media?.max_broll_entities ?? 4);
    for (const ent of uniqueEntities) {
      if (videoByEntity.has(ent)) continue;
      try {
        const clip = await sourceVideoBrollForEntity(ent, aspect, config, scratch, usedBrollUrls);
        if (clip) { videoByEntity.set(ent, clip); }
      } catch { /* fall back to stills for this entity */ }
    }
    if (videoByEntity.size > 0) {
      console.log(`[media-sourcing] segment ${segmentIndex}: ${videoByEntity.size} motion b-roll clip(s) for [${[...videoByEntity.keys()].join(', ')}]`);
    }
  }

  // Object-safe AI styles only. The old set included pencil/ink/watercolor,
  // which are "sketch" styles that actively invite Flux to draw a human figure
  // (owner QA 2026-08-14: a random female sketch appeared in a goldfish video).
  // Documentary photo + cinematic keep the frame on objects/places; abstract
  // subjects still get diagram/whiteboard. No sketch styles for scenes anymore.
  ...
  try {
    for (let b = 0; b < count; b++) {
      ...
      // ANIMATION-FIRST: if we have a motion clip for this beat's entity (or any
      // pooled clip), use it — real footage reads as far less "auto-generated"
      // than a Ken-Burns pan and is the owner's stated preference. Falls through
      // to stills below when no clip exists.
      const clip = videoByEntity.get(entity) || (videoByEntity.size > 0 ? [...videoByEntity.values()][b % videoByEntity.size] : null);
      if (clip?.buffer) {
        items.push({ ...clip, segmentIndex, durationSec: durations[b], seed: beatSeed, beatIndex: b, artStyle: 'broll-video' });
        continue;
      }

      // Reuse a verified REAL still (new Ken Burns) instead of a fresh AI frame
      // that could introduce a random face. A real primary is people-screened
      // already (see tryRealMediaForEntity), so reusing it is guaranteed safe.
      const canReuseReal = primaryAsset?.buffer && !primaryAsset.aiGenerated;
      if (config.media?.prefer_reuse_real_over_ai !== false && canReuseReal && b > 0 && b % 2 === 1) {
        items.push({ ...primaryAsset, segmentIndex, durationSec: durations[b], seed: beatSeed, beatIndex: b, artStyle: 'reuse-real-kb' });
        continue;
      }
      ...
      const style = kind === 'abstract'
        ...
        : sceneStyles[b % sceneStyles.length];
      ...
      if (config.media?.ai_fallback_enabled !== false) {
        ...
        // Screen the AI frame for a person. Diffusion still occasionally draws
        // a human even with object-only framing; this is the enforcement the
        // owner keeps asking for — vision-check EVERY AI beat, not just the
        // anchor. On a person (or a clear mismatch), replace it with a
        // guaranteed-safe reuse of the real primary rather than shipping a face.
        if (asset?.buffer && verifyAiBeats) {
          ...
          const nsfwBlocked = v?.checked && v.nsfw;
          const personBlocked = v?.checked && v.hasPerson;
          const irrelevant = v?.checked && v.relevant === false;
          if (nsfwBlocked || personBlocked || irrelevant) {
            const why = nsfwBlocked ? 'is NSFW' : personBlocked ? 'shows a person' : 'is irrelevant';
            console.warn(`[media-sourcing] AI beat ${b} for "${entity}" ${why} — replacing with real reuse/gradient`);
            // On NSFW, NEVER fall back to the AI frame: use a real reuse, else a
            // safe gradient (null buffer). Person/irrelevant can reuse real too.
            asset = canReuseReal ? { ...primaryAsset, artStyle: 'reuse-real-safe' } : null;
          }
        }
      }

      if (!asset?.buffer && primaryAsset?.buffer) {
        // Reuse the primary (even if AI) rather than a blank frame.
        asset = { ...primaryAsset, artStyle: canReuseReal ? 'reuse-real-safe' : 'reuse-primary' };
      }
