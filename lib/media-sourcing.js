...
export async function sourceMediaForSegment(segment, aspect, tmpDir, index, usedUrls) {
  ...
  // budget on one content-heavy video. It's also simply the wrong tool for
  // this channel's diagram/whiteboard visual style: an AI image generated
  // straight from the segment's own text is relevant by construction, so
  // there's nothing real to search for or verify against.
