 ...
 * as diagrams.
 */
function subjectKindFor(entity) {
  if (isAbstractConcept(entity)) return 'abstract';
  if (isPersonSubject(entity)) return 'person';
  // extractNamedEntities is the stricter, NER-backed signal. Only consulted
  // once the cheap vocabulary checks above have declined, so it can no longer
  // drag a Title Case technical term into the person path on its own.
  if (extractNamedEntities(entity).length > 0) return 'person';
  return 'scene';
}
...
function aiSubjectFor(entity, beatText, kind) {
  const context = String(beatText || '').trim().slice(0, 160);
  if (kind === 'abstract') {
    // Name the concept, then describe it. Nothing in this string can be
    // read as a request for a human figure.
    return `${entity}, illustrating: ${context}`;
  }
  if (kind === 'person') {
    // The likeness constraint lives in PERSON_SAFE_FRAMING in ai-image.js and
    // is phrased positively. Keep this string purely descriptive.
    return `a scene evoking "${entity}", context: ${context}`;
  }
...
export async function expandSegmentVisualBeats({
  ...
  for (let b = 0; b < count; b++) {
    ...
    const kind = subjectKindFor(entity);
    const style = styleForBeat(b, entity, kind);
    let asset = null;
