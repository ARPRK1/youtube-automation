...
4. Is the image not safe for a general audience? Mark nsfw TRUE if it contains ANY nudity, exposed or barely-covered genitals, buttocks, or female breasts/nipples, underwear/lingerie as the focus, sexual or suggestive content, explicit anatomical or medical close-ups of private body parts, gore, or graphic violence. Be STRICT and cautious -- if you are unsure whether it is appropriate for all ages, mark nsfw true.

Return ONLY JSON, no other text:
{"relevant": true/false, "hasWatermark": true/false, "hasPerson": true/false, "nsfw": true/false, "description": "one short sentence describing what's actually in the image"}`;
  ...
  try {
    ...
    if (!raw) return { relevant: true, hasWatermark: false, hasPerson: false, nsfw: false, description: 'unchecked (vision call returned nothing)', checked: false };
    try {
      const parsed = extractJson(raw);
      return { hasPerson: false, nsfw: false, ...parsed, checked: true };
    } catch (parseErr) {
      ...
      const nsfwMatch = raw.match(/"nsfw"\s*:\s*(true|false)/);
      if (relevantMatch) {
        return {
          ...
          // If the model flagged nsfw before truncating, honor it; if the field
          // itself is what got cut off, fail SAFE and treat as nsfw.
          nsfw: nsfwMatch ? nsfwMatch[1] === 'true' : true,
      ...
      return { relevant: false, hasWatermark: false, hasPerson: false, nsfw: true, description: `rejected -- verification response unparseable (${parseErr.message})`, checked: true };
    }
  } catch (err) {
    console.warn(`[vision-check] verification call failed, treating as unchecked: ${err.message}`);
    return { relevant: true, hasWatermark: false, hasPerson: false, nsfw: false, description: `unchecked (${err.message})`, checked: false };
  }
}
