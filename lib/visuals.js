...
export async function renderVisualTimeline({ items, aspect, outPath, seed = 'timeline' }) {
  ...
  try {
    ...
    for (let i = 1; i < clipPaths.length; i++) {
      ...
      // Every raw clip gets settb applied (see COMMON_TIMEBASE), but
      // xfade/concat's own OUTPUT doesn't necessarily preserve that
      // explicit timebase -- confirmed live on a real 14-segment video: a
      // hard-cut (concat filter) earlier in the chain produced an output
      // whose timebase no longer matched a later xfade partner, even
      // though every individual source clip was correctly normalized.
      // Re-assert settb after every stage, not just on the raw inputs.
      const rawOut = `[m${i}raw]`; // intermediate label so settb can run on the transition's own output
      const out = isLast ? '[outv]' : `[m${i}]`;
      if (transitions[i - 1] === 'fade') {
        const offset = Math.max(0, runningTotal - XFADE_OVERLAP_SEC);
        filterParts.push(`${current}${next}xfade=transition=fade:duration=${XFADE_OVERLAP_SEC}:offset=${offset.toFixed(3)}${rawOut}`);
        ...
        filterParts.push(`${current}${next}concat=n=2:v=1:a=0${rawOut}`);
        runningTotal += items[i].durationSec;
      }
      filterParts.push(`${rawOut}${COMMON_TIMEBASE}${out}`);
      current = out;
    }
