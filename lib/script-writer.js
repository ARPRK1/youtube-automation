...
async function generateNarration(research, structure, minWords) {
...
It MUST be at least ${minWords} words -- this is a hard requirement. ${extra}`;

  let narration = await callFreeLLM(basePrompt(''), 3000);
  if (wordCount(narration) < minWords) {
    narration = await callFreeLLM(
      basePrompt(`Your previous attempt was too short. Elaborate significantly more with extra detail, examples, and narrative beats until you clearly exceed ${minWords} words.`),
      3800
    );
  }
  ...
  if (repeated.length > 0) {
    const retry = await callFreeLLM(
      basePrompt(`Your previous attempt repeated itself -- these phrases each appeared 3+ times: ${repeated.slice(0, 3).join(' | ')}. This usually means the topic doesn't have enough real substance for the target length. Do not repeat any point, phrase, or beat -- if you're running out of genuinely new material, go deeper into specific detail/examples instead of restating earlier points differently.`),
      3800
    );
    const retryRepeats = findRepeatedPhrases(retry);
    if (retryRepeats.length === 0 && wordCount(retry) >= minWords) {
      narration = retry;
      repeated = [];
    }
  }
...
export async function writeLongScript(research) {
  ...
  const narration = await generateNarration(research, structure, Math.max(minWords, Math.round(targetWords * 0.6)));
  if (wordCount(narration) < minWords) {
