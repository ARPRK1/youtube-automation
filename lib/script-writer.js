...
export async function writeShortScripts(research, longScript, count) {
  ...
  const prompt = `You are writing YouTube Shorts for ModernMonk — an India food-origin story channel (faceless, Shorts-first). The owner's own cloned voice will narrate — write lines that sound like a sharp friend telling a secret, NOT a news anchor or textbook.
Winners on this channel: 28–45s food myth-busts with a shock hook in sentence 1 (e.g. "Biryani not Indian?" ~789 views).
Topic: "${research.topic}"
${longContext}
${factsBlock(research.facts)}
...
Each Short MUST cover a DIFFERENT angle of the topic (origin myth, city claim, ingredient, travel history, modern twist) — never rewrite the same hook ${count} times with new titles.
${growthExtra}
Voice: conversational, contractions, short sentences, no AI-cliché phrases ("delve", "in conclusion", "buckle up", "little did they know", "whether you're a... or a..."), no em-dash overuse. Prefer concrete Indian names, dishes, places, years.
