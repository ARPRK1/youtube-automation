...
export async function writeShortScripts(research, longScript, count) {
  ...
  const prompt = `You are writing YouTube Shorts for ModernMonk — a GLOBAL curiosity channel (facts, Top 5/10, history twists, science "wait what?", riddles). Faceless, Shorts-first. Owner's cloned voice narrates — sound like a sharp friend, NOT a textbook or AI narrator.
Topic: "${research.topic}"
${longContext}
${factsBlock(research.facts)}
Write ${count} standalone Shorts (9:16 vertical, under ${maxSeconds}s spoken ≈ ${maxShort} words max, at least ~${minShort} words). Target ~90–100 words each.
Each Short MUST be a DIFFERENT angle (e.g. one countdown, one myth-bust, one single deep fact, one quiz) — never rewrite the same hook ${count} times.
${growthExtra}
Voice: conversational, contractions, short sentences. Banned phrases: "delve", "in conclusion", "buckle up", "little did they know", "whether you're a... or a...", "in today's video".

CRITICAL — visual_needs (these drive on-screen images and MUST match the spoken words):
- 3–5 concrete SHOWABLE things per Short: objects, places, animals, maps, inventions, foods, buildings, symbols.
- NEVER list people names, "person", "crowd", "man", "woman", "scientist portrait" — we ban random faces.
- Prefer: "ice cube", "airplane wing", "world map", "QWERTY keyboard", "desert dunes", "library shelves", "ship container".
