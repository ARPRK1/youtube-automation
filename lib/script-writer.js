...
Return ONLY JSON, no other text: {"concepts":[{"sentences":[1],"shots":["...","..."]}, ...]}`;
  ...
  let raw = [];
  try {
    const parsed = extractJson(await callFreeLLM(prompt, Math.max(1000, sentences.length * 70)));
    if (Array.isArray(parsed.concepts)) raw = parsed.concepts;
  } catch (err) {
    console.warn(`[script-writer] storyboard generation failed: ${err.message}`);
  }
  ...
  if (started.length === 0) return { sentences, concepts: [] };
