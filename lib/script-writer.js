...
export async function writeShortScripts(research, longScript, count) {
  ...
  const prompt = `You are writing YouTube Shorts for ModernMonk — an India-focused faceless growth channel (food origins, simple money, history twists, hidden places). Winners on this channel are 25–45 second food/history stories with a shock hook in sentence 1.
Topic: "${research.topic}"
${longContext}
${factsBlock(research.facts)}
Write ${count} standalone Shorts (9:16 vertical, under ${maxSeconds}s spoken ≈ ${maxShort} words max, at least ~${minShort} words). Target ~90 words.
${growthExtra}
Voice: conversational Indian-English friendly, contractions, no AI-cliché phrases ("delve", "in conclusion", "buckle up", "little did they know", "whether you're a... or a..."), no em-dash overuse. Each must work standalone. Prefer concrete Indian names, dishes, places, rupees, years.
