...
export async function writeShortScripts(research, longScript, count) {
  ...
  const prompt = `You are writing YouTube Shorts scripts derived from this long-form video's best hooks/moments for an India-focused growth channel (ModernMonk).
...
Return ONLY JSON, no other text:
{
  ...
  return parsed.shorts.slice(0, count).map((s) => {
    ...
    const hashtags = [...new Set([...(s.hashtags || []), 'Shorts', 'India'])];
