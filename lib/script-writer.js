...
function trimAfterSecondOccurrence(text, phrase, minWords) {
  ...
  const findFrom = (from) => {
    for (let t = from; t <= tokens.length - phraseTokens.length; t++) {
      let ok = true;
  ...
  const pos1 = findFrom(0);
  if (pos1 === -1) return null;
  const pos2 = findFrom(pos1 + phraseTokens.length);
  if (pos2 === -1) return null;

  const phraseEndChar = tokenSpans[pos2 + phraseTokens.length - 1].end;
