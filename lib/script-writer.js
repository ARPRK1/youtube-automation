 ...
 * one natural callback, dropping only the padding beyond that). Returns
 * null if the trim would fall below `minWords`, so the caller still has
 * a real failure signal rather than shipping something too short. */
function trimAfterSecondOccurrence(text, phrase, minWords) {
  const idx1 = text.toLowerCase().indexOf(phrase);
  if (idx1 === -1) return null;
  const idx2 = text.toLowerCase().indexOf(phrase, idx1 + phrase.length);
  if (idx2 === -1) return null;
  // Keep through the end of the sentence containing the second occurrence.
  const sentenceEnd = text.indexOf('.', idx2 + phrase.length);
  const cut = sentenceEnd === -1 ? idx2 + phrase.length : sentenceEnd + 1;
...
async function generateNarration(research, structure, minWords, targetWords) {
...
Style: conversational spoken English, contractions (it's, don't, you're), short punchy sentences, occasional rhetorical questions, a clear point of view / original take rather than a dry summary. This is a real script a human host would actually say out loud, not a Wikipedia article read aloud.
...
Aim for around ${targetWords} words, covering the topic with real depth and specific detail -- but it MUST be at least ${minWords} words, and it is far more important to never repeat a point than to hit the target: if the topic runs out of genuinely new material before ${targetWords} words, a shorter script that stays fresh throughout beats a longer one that starts repeating itself. ${extra}`;
  ...
  if (repeated.length > 0) {
    // Last resort before giving up entirely: keep the first genuine use
    // of the repeated phrase plus one natural callback, and cut the rest
    // of the padding -- a shorter, clean video beats losing it outright.
    const trimmed = trimAfterSecondOccurrence(narration, repeated[0], minWords);
    if (trimmed) {
      console.warn(`[script-writer] trimmed narration after repeated phrase "${repeated[0]}" instead of failing outright (${wordCount(narration)} -> ${wordCount(trimmed)} words)`);
      narration = trimmed;
    } else {
...
const METADATA_SHAPE = `{
  ...
  "hashtags": ["3 to 6 short trending-style hashtags WITHOUT the # symbol"]
}`;
...
export async function writeLongScript(research) {
  ...
  const narration = await generateNarration(research, structure, minWords, targetWords);
  if (wordCount(narration) < minWords) {
    throw new Error(`Generated narration is too short (${wordCount(narration)} words, need ${minWords}+) -- refusing to proceed with a thin script`);
  }
...
${METADATA_SHAPE}`;
  ...
  const hashtags = Array.isArray(meta.hashtags) && meta.hashtags.length > 0 ? meta.hashtags : [];
...
export async function writeShortScripts(research, longScript, count) {
  ...
  const prompt = `You are writing YouTube Shorts scripts derived from this long-form video's best hooks/moments.
Long-form title: "${longScript.title}"
Topic: "${research.topic}"
${factsBlock(research.facts)}
Write ${count} standalone short-form scripts (9:16 vertical, under ${maxSeconds} seconds each, so under ${maxWords} words each), each with its own strong hook in the first sentence. Same voice/style rules as before: conversational, contractions, no AI-cliché phrases ("delve", "in conclusion", "buckle up", "little did they know", "whether you're a... or a..."), no em-dash overuse. Each must work standalone without having seen the long video.
...
Return ONLY JSON, no other text:
{
  "shorts": [
    {
      ...
      "narration": "spoken script, under ${maxWords} words",
  ...
  return parsed.shorts.slice(0, count).map((s) => {
    const narration = stripBannedPhrases(s.narration || '');
    const hashtags = [...new Set([...(s.hashtags || []), 'Shorts'])];
    return {
      ...s,
      narration,
      hashtags,
      description: assembleDescription(narration.split(/[.!?]/)[0] || s.title, hashtags)
    };
  });
}
