...
/**
 * Builds a concept STORYBOARD (owner 2026-08-14): instead of a new image per
 * sentence (which flashes and feels random), the narration is broken into
 * CONCEPTS — each a thing/place/subject the script dwells on for a stretch —
 * and each concept gets an ORDERED shot sequence that tells its mini-story
 * (establishing/wide → specific/detail), e.g. Taj Mahal → [India map w/ Agra
 * pinned, Taj Mahal wide, Taj Mahal marble detail]. media-sourcing then holds
 * that concept's shots over the span where it's actually discussed, so the
 * video reads like a story that follows the words.
 *
 * Named subjects are allowed ("Lionel Messi jersey") — only generic/random
 * person queries ("a man", "crowd") are dropped, and NSFW is always dropped.
 * Returns { concepts: [{ start_cue, shots: [...] }] } in narration order;
 * empty on failure so the caller falls back cleanly.
 */
export async function generateStoryboard(text, topic, { neverPeople = true } = {}) {
  const sentences = splitSentences(text);
  if (sentences.length === 0) return { sentences: [], concepts: [] };

  const numbered = sentences.map((s, i) => `${i + 1}. ${s}`).join('\n');
  const prompt = `You are the VISUAL DIRECTOR for a faceless video about "${topic}". The narration is below, one numbered sentence per line. Group CONSECUTIVE sentences into CONCEPTS — a concept is one thing/place/person/idea the narration focuses on for a stretch (e.g. each of the "5 places", each list item, or one distinct claim). The result should feel like a STORY that follows the words, not random images.

For EACH concept, in order, give:
- "sentences": the list of sentence NUMBERS it covers (consecutive, e.g. [2,3]). Every sentence must belong to exactly one concept; concepts must be in order and cover all ${sentences.length} sentences.
- "shots": an ORDERED list of 2-4 concrete, searchable visuals that build this concept from ESTABLISHING/wide to SPECIFIC/detail. Real stock footage/photos: objects, places, animals, landmarks, maps, buildings, or a specific NAMED subject when the concept is about them.

Example concept: {"sentences":[2,3],"shots":["map of India with Agra marked","Taj Mahal wide view at sunrise","Taj Mahal white marble dome close up"]}

Rules for every shot: no random strangers' faces or crowds, no readable text/watermarks, nothing anatomical/medical/explicit. A specific named person or team IS allowed (e.g. "Lionel Messi in jersey").

Sentences:
${numbered}

Return ONLY JSON, no other text: {"concepts":[{"sentences":[1],"shots":["...","..."]}, ...]}`;

  // Generous budget: gpt-oss is a reasoning model, so it spends tokens
  // "thinking" BEFORE the JSON — a tight budget truncated the concepts array
  // mid-output (confirmed live 2026-08-18: 13x "No JSON found"). Leave ample
  // room for reasoning + the full JSON.
  const budget = Math.max(2600, sentences.length * 170);
  let raw = [];
  for (let attempt = 0; attempt < 2 && raw.length === 0; attempt++) {
    try {
      const parsed = extractJson(await callFreeLLM(prompt, budget));
      if (Array.isArray(parsed.concepts)) raw = parsed.concepts;
    } catch (err) {
      console.warn(`[script-writer] storyboard generation failed (attempt ${attempt + 1}): ${err.message}`);
    }
  }

  // Screen shots and find each concept's starting sentence (0-based).
  const started = [];
  for (const c of raw) {
    const shots = (Array.isArray(c?.shots) ? c.shots : [])
      .map((s) => String(s || '').replace(/\([^)]*\)/g, '').trim())
      .filter((s) => s && !NSFW_VISUAL_RE.test(s) && !(neverPeople && PERSON_VISUAL_RE.test(s)) && isShowable(s))
      .slice(0, 4);
    if (shots.length === 0) continue;
    const nums = (Array.isArray(c?.sentences) ? c.sentences : []).map((n) => Number(n) - 1).filter((n) => Number.isInteger(n) && n >= 0 && n < sentences.length);
    if (nums.length === 0) continue;
    started.push({ shots, start: Math.min(...nums) });
  }
  started.sort((a, b) => a.start - b.start);
  if (started.length === 0) {
    // LLM gave nothing usable — DON'T collapse to the old 2-image beat logic.
    // Fall back to a per-sentence storyboard: one AI-generatable visual derived
    // from each sentence, so the concept engine still runs and the visuals stay
    // varied and script-matched (owner: "AI visuals based on script").
    console.warn('[script-writer] storyboard empty — using per-sentence AI fallback so visuals stay varied');
    return { sentences, concepts: sentences.map((s, i) => ({ shots: [fallbackVisualFromSentence(s, topic)], sIdxs: [i] })) };
  }
  if (started[0].start > 0) started[0].start = 0; // make sure sentence 0 is covered

  // Assign every sentence to the most recent concept that started at/before it.
  // This is robust to the model's ranges overlapping or leaving gaps — coverage
  // is always complete, contiguous, and in order (no dropped/absorbed concepts).
  const concepts = started.map((c) => ({ shots: c.shots, sIdxs: [] }));
  let ci = 0;
  for (let s = 0; s < sentences.length; s++) {
    while (ci + 1 < started.length && started[ci + 1].start <= s) ci++;
    concepts[ci].sIdxs.push(s);
  }
  return { sentences, concepts: concepts.filter((c) => c.sIdxs.length > 0) };
}
...
export async function writeLongScript(research) {
  ...
  const rawSegments = await Promise.all((await segmentNarration(narration)).map(async (seg) => ({
    ...
    // Concept storyboard for this chapter so the long video's visuals tell a
    // story that follows the words (same model as Shorts).
    storyboard: await generateStoryboard(seg.text, research.topic, { neverPeople })
  })));
...
Add genuine substance: a concrete example, the mechanism/why, or a surprising specific — never filler or repetition. Spoken style, short complete sentences, a real hook first, and a COMPLETE closing line (full sentence ending in . ! or ?). Output ONLY the narration text, no title, no quotes, no markdown.`;
  ...
  return Promise.all(parsed.shorts.slice(0, count).map(async (s) => {
    ...
    // Concept storyboard — each concept (thing/place/subject the narration
    // dwells on) gets an ordered shot sequence shown over the span where it's
    // discussed, so the visuals tell a story that follows the words.
    const storyboard = await generateStoryboard(narration, research.topic, { neverPeople });
