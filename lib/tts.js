...
// straightforward. Two providers, chosen in config.yaml (voice.provider):
...
//   worth having as an option. Both were verified to actually run.
...
import { isVoiceCloneEnabled, convertToClonedVoice } from './voice-clone.js';
 ...
 * durationSec, hadPause }. `srtPath` is null for Kokoro (no native word
 * timing -- lib/captions.js falls back to even distribution across the
 * known real duration in that case).
 */
export async function synthesizeSegmentAudio(text, outDir, baseName) {
  const config = loadConfig();
  const provider = config.voice?.provider || 'edge-tts';
  ...
  // Tone-color conversion happens on the raw synthesized speech, before the
  // reveal-pause silence is added -- the converter only ever needs to see
  // actual speech content, and there's no reason for it to touch inserted
  // silence.
  if (isVoiceCloneEnabled(config)) audioPath = await convertToClonedVoice({ inputPath: audioPath, config });
  if (isReveal) audioPath = await prependSilence(audioPath, 450);

  const durationSec = await probeDurationSeconds(audioPath);
  return { audioPath, srtPath: result.srtPath, durationSec, hadPause: isReveal };
}
