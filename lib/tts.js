...
export async function synthesizeSegmentAudio(text, outDir, baseName, opts = {}) {
  ...
  try {
    if (provider === 'chatterbox') {
      ...
      // edge-tts: negative rate = slower. For Shorts, use the configured MALE
      // voice (the consistent fallback the owner can switch to instead of the
      // clone); long-form keeps the standard edge voice.
      const baseSlow = kind === 'short' ? -12 : -4; // ~12% slower on Shorts
      const edgeVoice = kind === 'short'
        ? (config.voice?.edge_tts_male_voice || config.voice?.edge_tts_voice || 'en-US-ChristopherNeural')
        : (config.voice?.edge_tts_voice || 'en-US-AndrewNeural');
