...
export async function uploadVideo({
  ...
  const cat = categoryId
    || (kind === 'short'
      ? (config.upload?.category_id_short || '24')
      : (config.upload?.category_id_long || '27'));
  const madeForKids = config.upload?.made_for_kids === true;

  const auth = getOAuthClient();
  const youtube = google.youtube({ version: 'v3', auth });

  // Ensure Shorts shelf classification for vertical short-form.
  let finalTitle = String(title || 'Untitled').slice(0, 100);
  if (kind === 'short' && !/#shorts/i.test(finalTitle)) {
    finalTitle = `${finalTitle.replace(/\s*#Shorts\b/gi, '').trim()} #Shorts`.slice(0, 100);
  }
  ...
  const res = await youtube.videos.insert({
    ...
    requestBody: {
      ...
      // madeForKids MUST stay false for growth (kids audience kills comments,
      // notifications, and most ad personalization — confirmed channel-level
      // damage on ModernMonk before the 90-day push fix).
      status: {
