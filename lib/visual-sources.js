 ...
 * priority order: Wikimedia first (best for named entities), then Pexels/
 * Pixabay (best generic b-roll, video preferred over stills), then
 * Openverse. Each group is itself an array of candidates (richest first)
 * so the caller can vision-verify and fall through both within and across
 * sources instead of committing to the first technically-licensed hit.
 */
export async function fetchRealMediaCandidates(query, aspect) {
  ...
  return [wikimedia, [...pexelsVideo, ...pixabayVideo], [...pexelsPhoto, ...pixabayPhoto], openverse].filter((g) => g.length > 0);
}
