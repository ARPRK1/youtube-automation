...
/** Pexels' own search relevance isn't reliable either — a "cricket" chapter
 * query once matched a Varanasi cremation-ghat clip. Pexels videos have no
 * description field, but their page URL is a readable slug (e.g.
 * ".../casual-cricket-match-in-lush-green-park-30172302/"); extract that
 * plus any tags and run it through the same relevance check as the other
 * sources. */
...
export async function searchPexelsVideo(query, aspect) {
  ...
  const data = await safeFetchJson(
    `https://api.pexels.com/videos/search?query=${encodeURIComponent(query)}&orientation=${orientation}&per_page=6`,
    { headers: { Authorization: apiKey } }
  );
  const video = (data?.videos || []).find((v) => isRelevantTitle(query, pexelsVideoLabel(v)));
...
export async function searchPexelsPhoto(query, aspect) {
  ...
  const photo = (data?.photos || []).find((p) => isRelevantTitle(query, p.alt));
